...
from main.trainer.rewards_videor1 import combined_reward
...
def main():
    ...
    ap.add_argument("--accuracy_reward_weight", type=float, default=1.0)
    ap.add_argument("--format_reward_weight", type=float, default=1.0)
    ap.add_argument("--ptype_penalty_weight", type=float, default=1.0)
    ap.add_argument("--pneg_penalty_weight", type=float, default=1.0)
    args = ap.parse_args()
    ...
    for epoch in range(args.epochs):
        pbar = tqdm(range(len(ds)), desc=f"Stage2-VideoR1 epoch {epoch}")
        for i in pbar:
            ...
            reward_detail = combined_reward(
                pred,
                answer,
                meta=meta,
                accuracy_weight=args.accuracy_reward_weight,
            ...
            reward_rev_detail = combined_reward(
                pred_rev,
                answer,
                meta=meta,
                accuracy_weight=args.accuracy_reward_weight,
            ...
            reward_main = reward_detail["total"]
            reward_rev = reward_rev_detail["total"]
            reward_ema = (1 - ema_alpha) * reward_ema + ema_alpha * float(reward_main)
            penalties = compute_penalties(float(reward_main), float(reward_rev), reward_ema)
            reward_eff = (
                float(reward_main)
                - args.ptype_penalty_weight * penalties["ptype"]
                - args.pneg_penalty_weight * penalties["pneg"]
            )
            ...
            pbar.set_postfix(
                {
                    "acc": f"{reward_detail['accuracy']:.3f}",
                    "fmt": f"{reward_detail['format']:.3f}",
                    "reward": f"{reward_eff:.3f}",
    ...
    logger.info("Stage2 Video-R1-style training done.")
